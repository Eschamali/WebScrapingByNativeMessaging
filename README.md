...
2. 呪いを解く: Power Automate Desktop 本体を再起動します。（前述の通り、これでレジストリが正規のパスに修復されます）
...
### なんか、batファイルにウイルスあるって言われたんだけど大丈夫？😡

ファイルの中身を見ればわかる通りやってることはただの、このツールを新規プロセスとして起動するコマンドだけです。  
しかし残念ながら、`.bat`ファイルから他プログラムを叩く挙動は、マルウェアの「ダウンローダー」や「ランサムウェア」の挙動と似ている（難読化もしやすい）ため、最近のセキュリティソフト（特に対侵入検知が強いもの）はかなり過敏に反応します🥲  
今回、これに対処した、`ver0.1.2`をリリースし、`Excel.exe`を直接起動するオプションを追加しました。チェックを入れれば、検知は回避できますが、この`ReadMe`にも触れた下記の**致命的な面倒要素**に対処する必要が出ます。
![ウイルス回避モード](doc/AddStep.png)

* **全てのExcelをいったん閉じてください**  
  Excelは、起動中のプロセスに吸収してしまう仕様上、パイプ継承に失敗します。

* **引数の誤認**  
![引数エラーメッセージ](doc/Error.png)  
このようなエラーが出ますが、Excelになんらかの不安定なことをするものではないので、ご安心ください。  
OKを押下後、このツールを手動で開いてください。
